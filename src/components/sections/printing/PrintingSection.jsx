...
export default function PrintingSection() {
  ...
  return (
      ...
      <section className="relative py-5 md:py-10 overflow-hidden">
        <div className="container">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            ...
            >
              {/* Main Heading */}
              <h2 className="text-white font-bold leading-[1.1] text-3xl sm:text-4xl ">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                {/* Contact Now Button → /contactUs */}
                <Link to="/contactUs">
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="group px-4 py-4 rounded-full 
      bg-gradient-to-r from-[var(--color-tertiary)] to-[var(--color-primary)] 
      text-sm flex items-center justify-center gap-2
      transition-all duration-300"
                  >
                    Contact Now
                    <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform arrow-bounce" />
                  </motion.button>
                </Link>

                {/* Explore More Button → /services */}
                <Link to="/services">
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="group px-4 py-4 rounded-full border border-yellow-400 
    ...
    "
                >
                  <div
                    className="absolute inset-0 rounded-3xl p-[6px]"
                    style={{
                      background:
                  ...
                  >
                    <div className="w-full h-full rounded-[20px] bg-black/80" />
                  </div>
                </div>
              </div>
            </motion.div>
